...
const bindings=bindingDoc.bindings||{}; const allowedTargets=new Set(['clinic','material','semantic']);
...
for(const [name,meta] of Object.entries(bindings)){if(!tokenPaths.has(name))throw new Error(`orphan binding ${name}`);if(!allowedTargets.has(String(meta.target||'')))throw new Error(`unknown target ${name}`);if('phase'in meta||'sequence'in meta||'important'in meta)throw new Error(`historical binding metadata ${name}`);if(!/^--pt-(?:ref|sys|cmp|runtime|presentation)-[a-z0-9-]+$/i.test(String(meta.cssName||'')))throw new Error(`noncanonical cssName ${name}:${meta.cssName}`);}
...
process.stdout.write(`design-tokens-contract: dtcg=2025.10 resolver=1 sources=${files.length} tokens=${tokenPaths.size} bindings=${Object.keys(bindings).length} residual=0 historical-metadata=0 parallel-namespaces=0\n`);
