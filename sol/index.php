...
function walletSnapshot(): array
{
    $settings = WalletState::read();
    $address = (string)$settings['address'];
    $balances = $address === '' ? [
        'sol_balance' => 0.0,
        'usdc_balance' => 0.0,
        'stake_balance' => 0.0,
        'perps_status' => 'unavailable',
    ] : array_merge(SolanaWallet::balances($address), JupiterPortfolio::leverage($address));
    return [
        'ok' => true,
        'settings' => $settings,
        'balances' => $balances,
...
if ($api === 'wallet' && $_SERVER['REQUEST_METHOD'] === 'GET') {
    enforceWalletRateLimit();
    jsonResponse(walletSnapshot());
}
...
if ($api === 'wallet' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $integralizado = is_array($payload) && is_numeric($payload['integralizado_brl'] ?? null) ? max(0.0, (float)$payload['integralizado_brl']) : 0.0;
    if ($address !== '' && !preg_match('/^[1-9A-HJ-NP-Za-km-z]{32,44}$/', $address)) jsonResponse(['ok' => false, 'error' => 'invalid_address'], 400);
    WalletState::save($address, $integralizado);
    jsonResponse(walletSnapshot());
}

$assetVersion = max(array_map(static fn(string $file): int => (int)@filemtime(__DIR__ . '/' . $file), ['css/style.css','js/app-core.js','js/app-market.js','js/app-chart.js','js/app-portfolio.js','js/app-ui.js']));
