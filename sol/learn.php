...
if ($lock === null) {
    echo "SKIP scientific job ativo\n";
    exit(0);
}
...
try {
    ...
    while ($next < $target && microtime(true) - $started < 24) {
        ...
        if (!$batch) break;
        $history = CandleRepository::merge30m($history, $batch);
        $lastBatch = $batch[count($batch) - 1];
        $lastBatchTs = (int)($lastBatch['t'] ?? 0);
        if ($lastBatchTs < $next) break;
        $next = $lastBatchTs + 1800;
        $blocks++;
