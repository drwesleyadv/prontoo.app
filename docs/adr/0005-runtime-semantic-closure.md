...
Manter o monólito modular e perfilar Composition por responsabilidade. Cinco arquivos enumerados são composition roots concretos; os demais arquivos Runtime são bootstrap ou adapters finos de entrada. Runtime não contém SQL de negócio, PDO direto, adapters concretos fora dos roots nem transações de caso de uso. Domain/Core fornecem regras e invariantes, enquanto Infrastructure traduz essas decisões para SQL e implementa os ports de Application.
