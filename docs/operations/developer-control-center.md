...
- Todas as áreas HTML exibem duas áreas preenchidas no rodapé derivadas dos mesmos 30 pontos de Volume. Elas reutilizam a projeção `960×210`, a escala conjunta, os paddings e a baseline do gráfico, são pintadas na mesma ordem e não exibem contorno nem pontos. Apenas até 4 unidades horizontais ao redor de cada pico e vale recebem uma curva quadrática curta; o restante de cada encosta conserva as coordenadas de Volume. Nas superfícies públicas usam `#1f6f56` e `#347963` com opacidade `0.5`; nas autenticadas usam as cores de destaque do consultório ou do ambiente Desenvolvedor, também com `0.5`. Não há máscara de fade nem redesenho pelo cliente.
