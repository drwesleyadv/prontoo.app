...
- Todas as áreas HTML exibem duas ondas preenchidas no rodapé derivadas dos mesmos 30 pontos de Volume. A identidade visual é a usada antes da supressão do Status público: SVG `1000×250`, normalização conjunta, curvas Bézier cúbicas contínuas, fechamento até o fundo, `opacity: 0.5`, altura `15vh` limitada a `64–180px` e máscara vertical integral aos 34%. Carregamento de Páginas usa o papel cromático histórico de requisições — tom principal (`#347963` em público) — e Consulta o de registros — tom forte (`#1f6f56` em público). Nas áreas autenticadas, a mesma ordem usa as cores principal e forte do consultório ou do ambiente Desenvolvedor. A restauração é exclusivamente visual: a página Status continua suprimida, não há contagens públicas e o cliente não redesenha a geometria.
