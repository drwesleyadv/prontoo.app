...
Todas as áreas HTML exibem duas linhas decorativas de telemetria no rodapé, derivadas dos mesmos 30 pontos de Carregamento de Páginas e Consulta do gráfico Volume. A geometria é renderizada no servidor como uma silhueta normalizada: as encostas permanecem retas e somente os vértices são arredondados por curvas quadráticas, produzindo o perfil de montanhas. Em áreas públicas, o HTML contém apenas as coordenadas normalizadas do desenho, usa `#1f6f56` e `#347963` com opacidade `0.5` e nunca serializa contagens absolutas nem abre uma rota pública de refresh. Em áreas autenticadas, as linhas usam respectivamente as cores forte e principal de destaque do consultório. A rota histórica `login_telemetry_wave` permanece neutralizada; a rota `footer_telemetry_wave` permanece privada como fallback autenticado e ambas continuam excluídas da contagem de page loads.
