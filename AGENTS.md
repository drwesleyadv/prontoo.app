...
Todas as áreas HTML exibem no rodapé duas áreas decorativas derivadas dos mesmos 30 pontos de Carregamento de Páginas e Consulta do gráfico Volume. O renderer reutiliza exatamente o canvas `960×210`, os paddings `40/16/18/32`, a escala conjunta das duas séries, o arredondamento de coordenadas a duas casas e a ordem de pintura de Volume. As áreas fecham na mesma baseline e permanecem sem contornos ou pontos; somente uma faixa horizontal de até 4 unidades ao redor de cada pico ou vale é substituída por uma curva quadrática curta, mantendo os trechos restantes idênticos às encostas do gráfico. Em áreas públicas, as áreas usam `#1f6f56` e `#347963`, ambas com opacidade `0.5`; em áreas autenticadas, usam respectivamente as cores forte e principal do consultório, também com opacidade `0.5`. O rodapé não usa máscara de fade. `login_telemetry_wave` permanece neutralizada e `footer_telemetry_wave` permanece privada, mas o cliente não busca nem redesenha a geometria do rodapé.
