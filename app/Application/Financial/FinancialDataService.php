...
final class FinancialDataService
{
    public function __construct(
        private FinancialDataPort $port,
    ...
    public function ensureOperationalSchema(): void
    {
        $this->port->ensure('financial_operational');
    }

    public function ensureDailyClosingSchema(): void
    {
        $this->port->ensure('financial_daily_closing');
    }

    private function assertOperation(string $operation): void
    {
        if (preg_match('/^financial\.(?:0[1-9]|1[0-7])\.[a-z][a-z0-9_]*\.[0-9]{2}$/D', $operation) !== 1) {
            throw new InvalidArgumentException('Operação de persistência financeira inválida.');
        }
    }
}
