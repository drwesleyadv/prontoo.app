...
function prontoo_require_module(string $relative): void
{
    ...
    if (!is_file($file)) {
        ...
        throw new RuntimeException(
            "Módulo especializado essencial ausente: " .
                str_replace($root . "/", "", $file),
        );
    }
    $key = realpath($file) ?: $file;
    if (
        !isset($GLOBALS["PRONTOO_LOADED_MODULE_FILES"]) ||
        !is_array($GLOBALS["PRONTOO_LOADED_MODULE_FILES"])
    ) {
...
function prontoo_require_modules(array $files): void
{
    foreach ($files as $file) {
        prontoo_require_module($file);
    }
}
function prontoo_runtime_specialization_policy(): string
{
    return "pi-v2.4-light-runtime-route-loaded";
}
...
function prontoo_full_runtime_modules(): array
{
    return array_values(
        array_unique(
            array_merge(prontoo_runtime_core_modules(), [
                "Domain/Clinic/ClinicConfig.php",
                "Domain/Clinic/SubscriptionSettings.php",
...
function prontoo_load_runtime_core_modules(): void
{
    ...
    $done = true;
    prontoo_require_modules(
        prontoo_use_light_boot()
            ? prontoo_runtime_core_modules()
            : prontoo_full_runtime_modules(),
    );
}
...
{
    static $loaded = [];
    $groups = prontoo_route_module_groups($route);
...
function prontoo_route_module_groups(string $route): array
{
    ...
    $map = [
        ...
        "patient" =>
            $patients + $documents + $financial + $tasks + $appointments,
        ...
        "document_pdf_file" =>
            $documents + $patients + $financial + $appointments,
