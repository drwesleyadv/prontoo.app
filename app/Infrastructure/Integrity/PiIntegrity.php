...
namespace Prontoo\Infrastructure\Integrity;
...
final class PiIntegrity
{
    ...
    public static function configureRuntimeContext(array $context): void
    {
        self::$runtimeContext = [
    ...
    private static function flushEvents(bool $transactional): void
    {
        ...
        try {
            ...
            $payload = [
                ...
                'route' => (string) (self::$runtimeContext['route'] ?? ''),
                'clinic_id' => self::contextInt('clinic_id'),
                'user_id' => self::contextInt('user_id'),
            ...
            if ($ledgerId > 0) {
                ...
                $statement->execute([
                    $requestId,
                    self::contextInt('clinic_id'),
                    self::contextInt('user_id'),
                    mb_substr((string) ($payload['route'] ?? 'system'), 0, 80, 'UTF-8') ?: 'system',
                    'system_mutation',
                    'integrity',
                    'write',
                    'system',
                    (string) (self::$runtimeContext['role'] ?? ''),
